          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <label className="block text-sm font-medium text-green-800 mb-2">
                ✂️ Clipped Sources Data (JSON)
              </label>
              <textarea
                value={formData.sourcesJson || ''}
                onChange={(e) => setFormData({ ...formData, sourcesJson: e.target.value })}
                rows={6}
                placeholder='[{"id":"...", "box":...}]'
                className="w-full px-4 py-2 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white font-mono text-xs"
              />
              <p className="text-xs text-green-600 mt-2">
                This is the raw data generated by the Source Clipper. You can view or edit it here directly.
              </p>
            </div>
          </div>
        </div>
